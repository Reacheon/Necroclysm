...
public:
    ...
    Wave(int inputGridX, int inputGridY, int inputGridZ, int inputLifetime = 18)
    {
        ...
        if (list.size() > 500) dbgPrt(L"[메모리 누수 경고] Wave의 객체 수가 500개를 넘어갔습니다.\n");
        if (map[{ inputGridX, inputGridY, inputGridZ }].size() > 500) dbgPrt(L"[메모리 누수 경고] Wave의 객체 수가 한 타일에서 500개를 넘어갔습니다.\n");
    }
    ~Wave()
    {
