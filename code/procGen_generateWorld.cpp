...
//
//   ※ prt() 등 std::wprintf 계열은 스레드 안전이 보장되지 않으므로
//     워커에서는 호출하지 않음. 로그는 각 phase 함수 내부 prt가 처리.
//============================================================
namespace procGen
{
    namespace
    {
        ...
        //  (평균 안 함 — CityCenter 같은 단일 픽셀 마커가 사라지기 때문)
    ...
    void generateWorld(std::uint64_t seed, WorldGenProgress& progress)
    {
        ...
        //--- 미리보기 버퍼를 alpha=0(투명) 으로 미리 alloc → 즉시 표시 가능 상태로
        //    sectorLoadSink에서 섹터 1장씩 부분 갱신 → 위성이 그려지는 효과
        {
        ...
        // ╔═══════════════════════════════════════════════════════════╗
        // ║ DEBUG GUARD — 도시 분포 확인용. 도로망 알고리즘 작업 중   ║
        // ║   여기서 return하면 progress.done이 영원히 false 유지 →   ║
        // ║   WorldGenScreen은 자동으로 닫히지 않고 도시 점이 찍힌    ║
        // ║   상태로 멈춰서 표시됨. 종료는 창 닫기(QUIT)로만 가능.    ║
        // ║   ※ 도로망 작업 끝나면 이 블록 통째로 삭제할 것.          ║
        // ╚═══════════════════════════════════════════════════════════╝
        constexpr bool kDebugStopAfterCities = true;
        if constexpr (kDebugStopAfterCities)
        {
            return;
        }
