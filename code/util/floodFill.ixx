...
export int floodFillFindOrphan(const std::unordered_set<Point2,Point2::Hash>& inputConditions, Point2 startCoor, std::unordered_set<Point2, Point2::Hash>& outputSet)
{

	errorBox(inputConditions.find({ startCoor.x,startCoor.y }) == inputConditions.end(), L"플루드필의 시작점 조건이 거짓이다.");

	outputSet = inputConditions;
	outputSet.erase({ startCoor.x,startCoor.y });
	int floodNumber = 1; //색칠된 타일의 수
	std::queue<int> qX;
	std::queue<int> qY;
	...
	while (qX.empty() == false) //큐가 없어질 때까지 계속
	{
		...
		for (int i = 0; i < 8; i += 2)
		{
			...
			{
				{
					if (outputSet.find({ floodX + dx,floodY + dy }) != outputSet.end())
					{
						outputSet.erase({ floodX + dx,floodY + dy });
						floodNumber++;
						qX.push(floodX + dx);
						qY.push(floodY + dy);
					}
