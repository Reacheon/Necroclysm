...
namespace worldGen
{
    namespace
    {
        ...
        //후보 엣지 — 3-layer Gabriel 그래프(상세는 buildGabrielEdges 정의 참조).
        struct EdgeCand
        {
            int    a;
            int    b;
        ...
        //박스 마진으로부터 명시적 박스 — 짧은 엣지용 (boxFromMargin 정의 참조).
        ...
        //============================================================
        // 헬퍼 함수 forward declarations — 정의는 buildRoadNetwork 아래.
        //   Stepdown: 추상도 내림차순으로 정의됨
        //     pathOneEdge → astarPathBidir → astarCoarse
        //     → buildGabrielEdges → dedupEdges → boxFromMargin → rdpRecurse
        //============================================================
        std::vector<EdgeCand>   buildGabrielEdges(const std::vector<CityPixel>& cities, const SpatialHash& hash, TierMask mask, double maxDist);
        std::vector<EdgeCand>   dedupEdges(std::vector<EdgeCand> in);
        CoarseAStarOut          astarCoarse(int sx, int sy, int ex, int ey, const CoarseGrid& coarse, int margin, AStarWorkspace& ws);
        AStarOut                astarPathBidir(PixelCoord start, PixelCoord end, const PixelCostGrid& grid, int x0w, int y0w, int x1w, int y1w, AStarWorkspace& ws, const CorridorMask* mask);
        FineBox                 boxFromMargin(PixelCoord A, PixelCoord B, int marginPx) noexcept;
        void                    rdpRecurse(const std::vector<PixelCoord>& in, int lo, int hi, double eps2, std::vector<int>& keep);
        std::vector<PixelCoord> pathOneEdge(PixelCoord A, PixelCoord B, const PixelCostGrid& grid, const CoarseGrid& coarse, double directDistPx, AStarWorkspace& ws, std::uint64_t& fineExpanded, std::uint64_t& coarseExpanded, std::size_t& maxHeap);
    } // anonymous namespace
