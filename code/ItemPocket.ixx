...
public:
	...
	ItemPocket(storageType inputType) { type = inputType; }

	~ItemPocket(){prt(L"ItemPocket : 소멸자가 호출되었습니다..\n");}

	storageType getType() { return type; }
	...
	int addItemFromDex(int index) { return addItemFromDex(index, 1);}
	...
	void sortByUnicode() { sortByUnicode(0, itemInfo.size() - 1); }
	...
	void sortWeightDescend() { sortWeightDescend(0, itemInfo.size() - 1); }
	...
	void sortWeightAscend() { sortWeightAscend(0, itemInfo.size() - 1); }
	...
	void sortVolumeDescend() { sortVolumeDescend(0, itemInfo.size() - 1); }
	...
	void sortVolumeAscend() { sortVolumeAscend(0, itemInfo.size() - 1); }
	...
	int searchTxt(std::wstring inputStr) { return searchTxt(inputStr, 0, itemInfo.size() - 1); }
	...
	int searchFlag(itemFlag inputFlag) { return searchFlag(inputFlag, 0, itemInfo.size() - 1); }
	...
	int searchCategory(itemCategory input) { return searchCategory(input, 0, itemInfo.size() - 1); }
	...
	int searchSubcategory(itemSubcategory input) { return searchSubcategory(input, 0, itemInfo.size() - 1); }
