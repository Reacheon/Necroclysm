...
{
	static bool firstPlayerInput = true;
	static bool firstPlayerAnime = true;
	static bool firstMonsterAI = true;
	static bool firstMonsterAnime = true;
	...
	if (turnCycle == turn::playerInput)//턴사이클 0 : 플레이어의 입력을 받음. 받을 경우 1로 넘어감
	{
		if (firstPlayerInput)
		{
	...
	else if (turnCycle == turn::playerAnime || turnCycle == turn::monsterAnime)//엔티티들의 애니메이션 재생
	{
		//AniUSet
	...
	else if (turnCycle == turn::monsterAI)//모든 엔티티의 AI를 실행시킴
	{
		if (firstMonsterAI)
		{
			firstPlayerInput = true;
		...
		for (int i = 0; i < (World::ins())->getVehicleList().size(); i++)
		{
			if (((Vehicle*)(World::ins())->getVehicleList()[i])->runAI() == false) endMonsterTurn = false;
		}
		for (int i = 0; i < (World::ins())->getEntityList().size(); i++)
		{
			if ((World::ins())->getEntityList()[i] != (Player::ins()))
			{
				if (((Monster*)(World::ins())->getEntityList()[i])->runAI() == false) endMonsterTurn = false;
			}
		}
