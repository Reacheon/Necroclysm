...
namespace procGen
{
    namespace
    {
        ...
        constexpr double terrainWeight(Terrain t) noexcept
        {
            ...
            case Terrain::Subarctic:             return 0.3;
        ...
        //위도 belt LUT — py(0..H-1) → 위도 가중치(0.5..1.0).
        ...
        std::vector<double> buildLatitudeLut()
        {
            std::vector<double> lut(static_cast<std::size_t>(PixelCostGrid::H));
        ...
        int scanForWaterSq(int px, int py, const PixelCostGrid& grid) noexcept
        {
        ...
        struct SpatialHash
        {
            ...
            bool conflicts(int px, int py, int R, int /*unusedMaxR*/,
        ...
        DartResult placeTier(CityTier tier, int targetCount,
                             ...
                             const std::vector<double>& latLut,
                             const WaterBonusLut& waterLut,
                             const CitySink& onPlaced)
        {
            ...
            while (dr.placed < targetCount && dr.attempts < maxAttempts)
            {
                ...
                const double base = terrainWeight(t);
                if (base <= 0.0)
                {
                    ++dr.rejectTerrain;
                    continue;
                ...
                //2. 로컬 물 스캔 + 점수
                const int dSq = scanForWaterSq(px, py, grid);
    ...
    std::vector<CityNode> placeCities(std::uint64_t seed, const PixelCostGrid& grid, CitySink onPlaced)
    {
        ...
        const std::vector<double> latLut = buildLatitudeLut();   // H × 8B ≈ 168KB
