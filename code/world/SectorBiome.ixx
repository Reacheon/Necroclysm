...
// 청크 생성 시 해당 청크 중심이 위치한 픽셀의 바이옴을 조회해 기본 타일을 깔음
export class SectorBiome
{
private:
    std::array<std::array<chunkFlag, PIXEL_PER_SECTOR>, PIXEL_PER_SECTOR> pixelBiome{};

public:
    SectorBiome()
    {
        for (auto& row : pixelBiome) row.fill(chunkFlag::none);
    }
    ...
    void set(int localPixelX, int localPixelY, chunkFlag f)
    {
        ...
        pixelBiome[localPixelY][localPixelX] = f;
    }
    ...
    chunkFlag get(int localPixelX, int localPixelY) const
    {
        ...
        return pixelBiome[localPixelY][localPixelX];
    }
};
